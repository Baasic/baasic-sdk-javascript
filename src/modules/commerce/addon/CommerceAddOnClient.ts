...
import { TYPES as commerceTypes, CommerceAddOnRoute } from '../';

@injectable()
export class CommerceAddOnClient {

    get routeDefinition(): CommerceAddOnRoute {
        return this.commerceAddOnRoute;
    }

    constructor(
        @inject(commerceTypes.CommerceAddOnRoute) protected commerceAddOnRoute: CommerceAddOnRoute,
     ...
     * @method                         
     * @example commerceAddOnClient.find({   
                   pageNumber : 1,   
                   pageSize : 10,   
                   orderBy : '<field>',   
    ...
    **/
     ...
     * @method                        
     * @example commerceAddOnClient.get()
                      .then(function (data) {   
                          // perform success action here 
                      },
                       function (response, status, headers, config) {   
   ...
   **/
    get(id: string, options?: IGetRequestOptions): PromiseLike<IHttpResponse<any>> {
        return this.apiClient.get(this.commerceAddOnRoute.get(id, options));
    }

    /**                  
     * Returns a promise that is resolved once the create commerce action has been performed; this action creates a new commerce resource.                  
     * @method                         
     * @example commerceAddOnClient.create({   
                    name : '<product-name>',   
                    slug : '<slug>',   
     ...
     * @example // commerceProduct is a resource previously fetched using get action. 
                        ...
                        commerceAddOnClient.update(commerceProduct)
                            .then(function (data) {   
                                // perform success action here 
                            },
                             function (response, status, headers, config) {   
     ...
     * @method                         
     * @example // commerceProduct is a resource previously fetched using get action.				 
                    commerceAddOnClient.remove(commerceProduct)
                        .then(function (data) {   
                            // perform success action here 
                        },
                         function (response, status, headers, config) {   

/* globals module */
/**  
 * @module blogPostFilesBatchClient  
 * @description  Blog Post Files Batch Client provides Baasic route templates which can be expanded to Baasic REST URIs. Various services can use Baasic Files Route Service to obtain needed routes while other routes will be obtained through HAL. By convention, all route services use the same function names as their corresponding services. 
*/
...
@injectable()
export class BlogPostFilesBatchClient {

    get routeDefinition(): BlogPostFilesBatchRoute {
     ...
     * @example // Remove original file resources                
                   blogPostFilesBatchClient.unlink([{ id: '<file-id>' }])
                       .then(function (data) {   
               ...
               // Remove derived file resources  
                   blogPostFilesBatchClient.unlink([{ id: '<file-id>', fileFormat: { width: <width>, height: <height> } }])
                       .then(function (data) {   
                           // perform success action here 
                       },
                        function (response, status, headers, config) {   
    ...
    **/
    unlink(data: IBlogPostFile[]): PromiseLike<IHttpResponse<void>> {
        return this.apiClient.delete<void>(this.blogPostFilesBatchRoute.unlink(), undefined, data);
    }
     ...
     * @example blogPostFilesBatchClient.update(files)
                    .then(function (data) {   
                        // perform success action here 
                    },
                     function (response, status, headers, config) {   
     ...
     * @example blogPostFilesBatchClient.link(files)
                    .then(function (data) {   
                        // perform success action here 
                    },
                     function (response, status, headers, config) {   
     ...
     **/
    link(data: IBlogPostFile[]): PromiseLike<IHttpResponse<any>> {
